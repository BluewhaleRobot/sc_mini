...
namespace sc_m_c
{
	...
	float FilterRatio = 2.5; //滤波功能用到的滤波系数设置为2.5，数值越小滤波效果越强，数值要大于０
...
#define RANGES_TIMP_SIZE 500 //tof雷达改为720
	...
	int SCLaser::poll(sensor_msgs::LaserScan::Ptr scan,int fd)
	{
		...
		while(Rcv > 0)
		{
			if(M1C1_MiniData.BufferLen < M1C1_Mini_FRAME_LEN_MAX) //ÊýŸÝ³€¶ÈÌ«¶Ì£¬ŒÌÐøÊÕÊý
			{
				cpylen = Rcv > (M1C1_Mini_FRAME_LEN_MAX - M1C1_MiniData.BufferLen) ? (M1C1_Mini_FRAME_LEN_MAX - M1C1_MiniData.BufferLen) : Rcv;
				memcpy(&M1C1_MiniData.Buffer[M1C1_MiniData.BufferLen], serialport_databuf, cpylen);
				Rcv = Rcv - cpylen;
			...
			while(M1C1_MiniData.BufferLen >= M1C1_Mini_FRAME_LEN_MIN)
			{
				...
				&& (M1C1_MiniData.Frame.CSL == (M1C1_MiniData.Frame.PHL^M1C1_MiniData.Frame.CT^M1C1_MiniData.Frame.FSAL^M1C1_MiniData.Frame.LSAL^M1C1_MiniData.Frame.Si[0]))
				&& (M1C1_MiniData.Frame.CSH = (M1C1_MiniData.Frame.PHH^M1C1_MiniData.Frame.LSN^M1C1_MiniData.Frame.FSAH^M1C1_MiniData.Frame.LSAH^M1C1_MiniData.Frame.Si[1]))
				...
				)
				{
					...
					PutRemainder2Start(M1C1_MiniData.Buffer, M1C1_Mini_FRAME_LEN_MIN, M1C1_MiniData.BufferLen);
				...
				{
					if(M1C1_MiniData.BufferLen < 2*M1C1_MiniData.Frame.LSN+M1C1_Mini_HEAD_LEN)
					{
						break;
					}
					csl = M1C1_MiniData.Frame.PHL^M1C1_MiniData.Frame.CT^M1C1_MiniData.Frame.FSAL^M1C1_MiniData.Frame.LSAL;
					csh = M1C1_MiniData.Frame.PHH^M1C1_MiniData.Frame.LSN^M1C1_MiniData.Frame.FSAH^M1C1_MiniData.Frame.LSAH;
					...
					if(M1C1_MiniData.Frame.CSL == csl && M1C1_MiniData.Frame.CSH == csh)
					{
						...
						GyroYawBak = GyroYaw;
						DeltaYaw.GyroYaw = DeltaYaw.GyroYaw > 360 ? DeltaYaw.GyroYaw - 360 : DeltaYaw.GyroYaw;
						DeltaYaw.GyroYaw = DeltaYaw.GyroYaw < -360 ? DeltaYaw.GyroYaw + 360 : DeltaYaw.GyroYaw;
						ZoneStartGyro += DeltaYaw.GyroYaw;

						fStartAngle = (M1C1_MiniData.Frame.FSAL + M1C1_MiniData.Frame.FSAH*256)/128.0F;
						fStopAngle = (M1C1_MiniData.Frame.LSAL + M1C1_MiniData.Frame.LSAH*256)/128.0F;
						if(fStartAngle < 10)
						{
							AllAngleIndex = 0;
						}
						if(M1C1_MiniData.Frame.LSN > 1)
						{
							...
							{
								fIncAngle = (360 - fStartAngle + fStopAngle)/(M1C1_MiniData.Frame.LSN-1);

							}
							else
							{
								fIncAngle = (fStopAngle - fStartAngle)/(M1C1_MiniData.Frame.LSN-1);
						...
						for(int j=0; j<M1C1_MiniData.Frame.LSN && AllAngleIndex < ANGLE_MAX_NUMBERS; j++)
						{
							temp_depth = (M1C1_MiniData.Frame.Si[2*j] | (M1C1_MiniData.Frame.Si[2*j+1]<<8))/4;
							...
							*/
							//angle_c1 += zone_start_rotate_angle + j*fCompensateStepAngle + degree_cali;
						...
						M1C1_MiniData.BufferLen -= (2*M1C1_MiniData.Frame.LSN+M1C1_Mini_HEAD_LEN);
						PutRemainder2Start(M1C1_MiniData.Buffer, 2*M1C1_MiniData.Frame.LSN+M1C1_Mini_HEAD_LEN, M1C1_MiniData.BufferLen);
					}
					else
					{
						for(i=1; i < M1C1_MiniData.BufferLen-1; i++)
						{
							if(M1C1_MiniData.Buffer[i] == 0xAA && M1C1_MiniData.Buffer[i+1] == 0x55)
							{
								break;
							}
						}
